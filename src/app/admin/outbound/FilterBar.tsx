 ...
 *
 * Filters:
 *   - Date Range  (Tanggal field, YYYY-MM-DD)
 *   - STATUS      (multi-select, fixed: Muat Pagi | Muat Inap)
 *   - Search      (freightOrder | mobilMuat) — debounced 300ms
 */
...
// ─── STATUS options — only two valid values ───────────────────────────────────

const STATUS_OPTIONS = ["Muat Pagi", "Muat Inap"] as const;
...
function FilterBarInner({ filters, onChange, onReset }: FilterBarProps) {
    ...
    return (
        <div className="bg-white border border-[#E5E7EB] rounded-[18px] p-5 shadow-sm">
            ...
            {/* Filter controls — 3 columns on xl, 2 on sm */}
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <MultiSelect
                        ...
                        onChange={(s) => onChange({ selectedStatus: s })}
                    />
                </div>

                {/* Free-text search */}
                <div className="space-y-2">
                    <label htmlFor="outbound-search" className="block text-xs font-medium text-[#64748B]">
                        Pencarian
                    </label>
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Cari FREIGHT ORDER atau Mobil Muat..."
                        ...
                        {rawSearch && (
                            <button
                                type="button"
                                onClick={() => { setRawSearch(""); onChange({ searchQuery: "" }); }}
                ...
                {hasActive && (
                    ...
                    >
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {filters.selectedStatus.map((s) => (
                                <FilterChip
                                    key={s}
                                    label={`Status: ${s}`}
                                    onRemove={() => onChange({
                                        selectedStatus: filters.selectedStatus.filter((x) => x !== s),
                                    })}
                                />
                            ))}
                            {filters.searchQuery && (
