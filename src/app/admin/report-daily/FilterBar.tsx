...
/**
 * FilterBar — Report Daily Transport
 * Filters: Rentang Tanggal | Division | Jenis Report | Search
...
function SelectFilter({ label, id, value, options, placeholder = "Semua", onChange }: SelectProps) {
    return (
        <div className="space-y-2">
            <label htmlFor={id} className="block text-xs font-medium text-[#64748B]">{label}</label>
...
export const INITIAL_REPORT_DAILY_FILTERS: ReportDailyFilters = {
...
const DIVISION_OPTIONS: DivisionType[] = ["Transport", "Warehouse FG", "Warehouse BS"];

// ─── Props ────────────────────────────────────────────────────────────────────

interface FilterBarProps {
    filters: ReportDailyFilters;
    jenisReportOptions: string[];
    onChange: (partial: Partial<ReportDailyFilters>) => void;
    onReset: () => void;
}

// ─── Component ────────────────────────────────────────────────────────────────

function FilterBarInner({ filters, jenisReportOptions, onChange, onReset }: FilterBarProps) {
    ...
    const hasActive = useMemo(() => !!(
        ...
        filters.division || filters.jenisReport || filters.searchQuery
    ), [filters]);
    ...
    return (
        <div className="bg-white border border-[#E5E7EB] rounded-[18px] p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                {/* Division */}
                <SelectFilter label="Division" id="rd-division" value={filters.division}
                    options={DIVISION_OPTIONS} placeholder="Semua Division"
                    onChange={(v) => onChange({ division: v })} />

                {/* Jenis Report */}
                <SelectFilter label="Jenis Report" id="rd-jenis" value={filters.jenisReport}
                    options={jenisReportOptions} placeholder="Semua Jenis"
                    onChange={(v) => onChange({ jenisReport: v })} />

                {/* Search */}
                <div className="space-y-2">
                    <label htmlFor="rd-search" className="block text-xs font-medium text-[#64748B]">Pencarian</label>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#9CA3AF]" aria-hidden="true" />
                        <input id="rd-search" type="search" value={rawSearch}
                ...
                {hasActive && (
                    <motion.div key="chips" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {filters.division && (
                                <FilterChip label={`Division: ${filters.division}`} onRemove={() => onChange({ division: "" })} />
                            )}
                            {filters.jenisReport && (
                                <FilterChip label={`Jenis: ${filters.jenisReport}`} onRemove={() => onChange({ jenisReport: "" })} />
                            )}
                            {filters.searchQuery && (
