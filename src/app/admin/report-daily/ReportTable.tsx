...
/**
 * ReportTable — Tabel detail dengan kolom Action (Edit / Delete).
 *
 * Columns: Tanggal | Division | Jenis Report | Keterangan / Value | Informasi Tambahan | Aksi
...
const DIVISION_STYLES: Record<string, string> = {
    ...
    "Warehouse FG": "bg-blue-50 text-blue-700 border-blue-200",
    "Warehouse BS": "bg-violet-50 text-violet-700 border-violet-200",
};
...
type SortKey = keyof Pick<TransportRecord, "tanggal" | "division" | "jenisReport" | "keterangan">;
...
const TableRow = memo(function TableRow({
    ...
    return (
        ...
        >
            ...
            <td className="px-4 py-3"><DivisionBadge division={record.division} /></td>
            <td className="px-4 py-3 text-xs text-[#374151] whitespace-nowrap">{record.jenisReport}</td>
            <td className="px-4 py-3 text-xs font-semibold text-[#111827] max-w-[200px] truncate">
                {record.keterangan || "—"}
            </td>
            <td className="px-4 py-3 text-xs text-[#64748B] max-w-[180px] truncate">
...
export default function ReportTable({ data, onEdit, onDelete }: ReportTableProps) {
    ...
    const searched = useMemo(() => {
        ...
        return data.filter((r) =>
            ...
            r.jenisReport.toLowerCase().includes(q) ||
    ...
    return (
        <div className="bg-white border border-[#E5E7EB] rounded-[18px] overflow-hidden shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-[#E5E7EB]">
                <div>
                    <p className="text-sm font-bold text-[#111827]">Log Transaksi</p>
                    <p className="text-xs text-[#64748B] mt-0.5">
                        {sorted.length.toLocaleString("id-ID")} data
                    </p>
                </div>
                <div className="relative w-full sm:w-64">
                    ...
                    <input
                        ...
                        placeholder="Cari division, jenis, keterangan..."
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-5 py-3 border-t border-[#E5E7EB] bg-[#F9FAFB]">
                <div className="flex items-center gap-2 text-xs text-[#64748B]">
                    ...
                    <span>
                        {sorted.length > 0
                            ? `${start}–${end} dari ${sorted.length.toLocaleString("id-ID")}`
                            : "0 data"}
                    </span>
                </div>
                <div className="flex items-center gap-1">
                    ...
                    <span className="px-3 h-7 flex items-center text-xs text-[#374151] font-medium">
                        {safePage} / {totalPages}
                    </span>
                    {[
                        {
