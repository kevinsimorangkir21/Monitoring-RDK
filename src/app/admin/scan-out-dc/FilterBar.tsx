...
/**
 * FilterBar — Scan Out DC
 * Filters: Rentang Tanggal | Vendor | No Polisi (cascading dari Vendor) | Search
...
export interface ScanOutFilters {
...
export const INITIAL_SCAN_OUT_FILTERS: ScanOutFilters = {
...
interface FilterBarProps {
    filters: ScanOutFilters;
    vendors: string[];
    /** Nopol list — dikosongkan/difilter saat vendor berubah */
    nopolOptions: string[];
    onChange: (partial: Partial<ScanOutFilters>) => void;
    onReset: () => void;
}

// ─── Component ────────────────────────────────────────────────────────────────

function FilterBarInner({ filters, vendors, nopolOptions, onChange, onReset }: FilterBarProps) {
    ...
    // Saat vendor berubah, reset nopol juga
    const handleVendor = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
        onChange({ vendor: e.target.value, nopol: "" });
    }, [onChange]);

    const handleNopol = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
        onChange({ nopol: e.target.value });
    }, [onChange]);
    ...
    const hasActive = useMemo(() => !!(
        ...
        filters.vendor || filters.nopol || filters.searchQuery
    ), [filters]);
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-[18px] p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                    <label htmlFor="so-vendor" className="block text-xs font-medium text-[#64748B]">Vendor</label>
                    <div className="relative">
                        <select id="so-vendor" value={filters.vendor} onChange={handleVendor}
                ...
                {/* No Polisi (cascading) */}
                <div className="space-y-2">
                    <label htmlFor="so-nopol" className="block text-xs font-medium text-[#64748B]">No Polisi</label>
                    <div className="relative">
                        <select id="so-nopol" value={filters.nopol} onChange={handleNopol}
                            disabled={nopolOptions.length === 0}
                            className="w-full h-9 pl-3 pr-8 rounded-xl border border-[#E5E7EB] text-xs text-[#111827] bg-white outline-none focus:border-[#DC2626] focus:ring-2 focus:ring-red-100 appearance-none cursor-pointer disabled:bg-[#F9FAFB] disabled:text-[#9CA3AF] disabled:cursor-not-allowed">
                            <option value="">Semua Nopol</option>
                            {nopolOptions.map((n) => <option key={n} value={n}>{n}</option>)}
                        </select>
                        <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#9CA3AF] pointer-events-none" />
                    </div>
                    {filters.vendor && nopolOptions.length === 0 && (
                        <p className="text-[10px] text-[#9CA3AF]">Belum ada nopol untuk vendor ini</p>
                    )}
                </div>

                {/* Search */}
                <div className="space-y-2">
                    <label htmlFor="so-search" className="block text-xs font-medium text-[#64748B]">Pencarian</label>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#9CA3AF]" aria-hidden="true" />
                        <input id="so-search" type="search" value={rawSearch}
                ...
                {hasActive && (
                    <motion.div key="chips" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {filters.vendor && (
                                <FilterChip label={`Vendor: ${filters.vendor}`}
                                    onRemove={() => onChange({ vendor: "", nopol: "" })} />
                            )}
                            {filters.nopol && (
                                <FilterChip label={`Nopol: ${filters.nopol}`}
                                    onRemove={() => onChange({ nopol: "" })} />
                            )}
                            {filters.searchQuery && (
