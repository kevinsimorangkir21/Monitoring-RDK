...
/**
 * FilterBar — Filter controls for the Gantungan Faktur dashboard.
 *
 * Filters:
 *   - Date Range       (Tanggal field)
 *   - CUSTOMER         (multi-select)
 *   - PAY TERMS        (multi-select)
 *   - Search           (CUSTOMER | NAMA TOKO | SALES DOC)
 */
...
import type { FakturFilters } from "./types";
...
interface FilterBarProps {
    filters: FakturFilters;
    availableCustomers: string[];
    availablePayTerms: string[];
    onChange: (partial: Partial<FakturFilters>) => void;
    onReset: () => void;
}
...
export default function FilterBar({
    filters, availableCustomers, availablePayTerms, onChange, onReset,
}: FilterBarProps) {
    ...
    const hasActive = useMemo(() => !!(
        ...
        filters.selectedCustomer.length > 0 || filters.selectedPayTerms.length > 0 ||
        filters.searchQuery
    ), [filters]);
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-[18px] p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                    <span className="block text-xs font-medium text-[#64748B]">CUSTOMER</span>
                    <MultiSelect
                        placeholder="Pilih Customer..."
                        options={availableCustomers}
                        selected={filters.selectedCustomer}
                        onChange={(s) => onChange({ selectedCustomer: s })}
                    />
                </div>

                {/* PAY TERMS */}
                <div className="space-y-2">
                    <span className="block text-xs font-medium text-[#64748B]">PAY TERMS</span>
                    <MultiSelect
                        placeholder="Pilih Pay Terms..."
                        options={availablePayTerms}
                        selected={filters.selectedPayTerms}
                        onChange={(s) => onChange({ selectedPayTerms: s })}
                    />
                </div>

                {/* Search */}
                <div className="space-y-2">
                    <label htmlFor="faktur-search" className="block text-xs font-medium text-[#64748B]">Pencarian</label>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#9CA3AF]" />
                        <input id="faktur-search" type="search" value={rawSearch}
                            onChange={(e) => setRawSearch(e.target.value)}
                            placeholder="Customer / Nama Toko / Sales Doc"
                ...
                {hasActive && (
                    <motion.div key="chips"
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {filters.selectedCustomer.map((c) => (
                                <FilterChip key={c} label={c}
                                    onRemove={() => onChange({ selectedCustomer: filters.selectedCustomer.filter((x) => x !== c) })} />
                            ))}
                            {filters.selectedPayTerms.map((p) => (
                                <FilterChip key={p} label={`PT: ${p}`}
                                    onRemove={() => onChange({ selectedPayTerms: filters.selectedPayTerms.filter((x) => x !== p) })} />
                            ))}
                            {filters.searchQuery && (
