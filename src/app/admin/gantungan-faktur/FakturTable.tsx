...
/**
 * FakturTable — Paginated detail table for Gantungan Faktur.
 *
 * Columns (Excel-aligned):
 *   Tanggal | Date | PAY TERMS | CUSTOMER | NAMA TOKO |
 *   SD DOCUMENT | SALES DOC | NET VALUE | KETERANGAN DARI TRANSPORT | Aksi
 */
...
const COL_COUNT = 10; // 9 data columns + 1 action

function SkeletonRow() {
    return (
        <tr className="animate-pulse" aria-hidden="true">
            {Array.from({ length: COL_COUNT }, (_, i) => (
                <td key={i} className="px-3 py-3"><div className="h-4 bg-[#F3F4F6] rounded w-full" /></td>
            ))}
        </tr>
    );
}
        ...
        <tr>
            <td colSpan={COL_COUNT} className="py-16 text-center">
                <div className="flex flex-col items-center gap-3 text-[#9CA3AF]">
                    ...
                    <p className="text-xs">Tambahkan data menggunakan tombol &quot;+ Tambah Faktur&quot;</p>
                </div>
            </td>
        </tr>
    );
...
interface PaginationProps {
    ...
    onPageChange: (p: number) => void; onPageSizeChange: (s: PageSizeOption) => void;
}
    ...
    return (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-[#E5E7EB]">
            <div className="flex items-center gap-3 text-sm text-[#64748B]">
                <span>Tampilkan</span>
                <select value={pageSize} onChange={(e) => onPageSizeChange(Number(e.target.value) as PageSizeOption)}
...
const COLS: { label: string; align?: "right" }[] = [
    ...
    { label: "SD DOCUMENT" },
    { label: "SALES DOC" },
    { label: "NET VALUE", align: "right" },
    { label: "KETERANGAN DARI TRANSPORT" },
    { label: "Aksi" },
];
...
interface FakturTableProps {
    data: FakturRecord[];
    loading?: boolean;
    onEdit: (record: FakturRecord) => void;
    onDelete: (record: FakturRecord) => void;
}

export const FakturTable = memo(function FakturTable({
    ...
    return (
        <section
            ...
            aria-label="Tabel Gantungan Faktur"
        >
            <div className="p-5 border-b border-[#E5E7EB]">
                <h2 className="text-sm font-bold text-[#111827]">Detail Gantungan Faktur</h2>
                {!loading && (
            ...
            <div className="overflow-x-auto" role="region" aria-label="Scroll tabel">
                <table className="w-full min-w-[900px] text-sm" aria-label="Data gantungan faktur">
                    <thead className="bg-[#F9FAFB]">
                        <tr>
                            {COLS.map((col) => (
                                <th key={col.label} scope="col"
                                    className={`px-3 py-3 text-xs font-semibold text-[#64748B] uppercase tracking-wider whitespace-nowrap ${col.align === "right" ? "text-right" : "text-left"}`}>
                                    {col.label}
                                </th>
                            ))}
                        </tr>
                        ...
                        {loading ? (
                            ...
                            pageData.map((r) => (
                                <tr key={r.id} className="hover:bg-[#F9FAFB] transition-colors">
                                    <td className="px-3 py-3 text-[#64748B] whitespace-nowrap text-xs">{fmtDate(r.tanggal)}</td>
                                    <td className="px-3 py-3 text-[#64748B] whitespace-nowrap text-xs">{fmtDate(r.date)}</td>
                                    <td className="px-3 py-3 text-[#64748B] whitespace-nowrap text-xs font-medium">{r.payTerms}</td>
                                    <td className="px-3 py-3 font-medium text-[#111827] whitespace-nowrap text-xs max-w-[140px] truncate" title={r.customer}>{r.customer}</td>
                                    <td className="px-3 py-3 text-[#64748B] whitespace-nowrap text-xs max-w-[120px] truncate" title={r.namaToko}>{r.namaToko}</td>
                                    <td className="px-3 py-3 text-[#64748B] whitespace-nowrap text-xs font-mono">{r.sdDocument}</td>
                                    <td className="px-3 py-3 text-[#64748B] whitespace-nowrap text-xs font-mono">{r.salesDoc}</td>
                                    <td className="px-3 py-3 text-[#111827] whitespace-nowrap text-xs font-semibold text-right">{fmtRp(r.netValue)}</td>
                                    <td className="px-3 py-3 text-[#64748B] text-xs max-w-[160px] truncate" title={r.keteranganTransport}>{r.keteranganTransport || "—"}</td>
                                    <td className="px-3 py-3 whitespace-nowrap">
                                        <div className="flex items-center gap-1">
                                            <button type="button" onClick={() => onEdit(r)}
                                                aria-label={`Edit ${r.salesDoc}`}
