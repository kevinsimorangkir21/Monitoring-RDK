 ...
 * Shows full faktur information, status timeline, Print / Download PDF / Close.
 */

import { useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
    X, Hash, Building2, FileText,
    DollarSign, Calendar, Truck,
    CheckCircle2, Clock, AlertCircle,
    ClipboardList, Download, Printer,
    FileCheck, FileClock, MapPin,
} from "lucide-react";
...
export default function DetailDrawer({ record, onClose }: Props) {
    ...
    const isOutstanding = record?.status === "Outstanding";
    ...
    const steps: Step[] = record
        ? [
            {
                label: "Faktur Dibuat",
                time: fmtTs(record.fakturDibuat),
                done: true,
                icon: ClipboardList,
            },
            {
                label: "Diajukan ke Proses",
                time: fmtTs(record.diajukan),
                done: isPending || isCompleted,
                icon: FileClock,
            },
            {
                label: "Sedang Diproses",
                time: fmtTs(record.diProses),
                done: isCompleted,
                icon: FileText,
            },
            {
    ...
    const bannerStyle = isCompleted
        ? "bg-emerald-50 border-emerald-200"
        : isOutstanding
            ? "bg-red-50 border-red-200"
            : "bg-amber-50 border-amber-200";

    const bannerIcon = isCompleted
                ...
                <>
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E5E7EB] bg-[#F9FAFB] shrink-0">
                            <div>
                                <p className="text-sm font-bold text-[#111827]">Detail Gantungan Faktur</p>
                                <p className="text-[11px] text-[#64748B] mt-0.5">{record.nomorFaktur}</p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                            ...
                            <div className={`flex items-center gap-3 p-3 rounded-xl border ${bannerStyle}`}>
                                {bannerIcon}
                                <div>
                                    <p className={`text-xs font-bold ${bannerTextColor}`}>
                                        {record.status}
                                    </p>
                            ...
                            <div>
                                ...
                                <Row label="Nomor Invoice" value={record.nomorInvoice} icon={Hash} />
                                <Row label="Nomor Faktur" value={record.nomorFaktur} icon={FileText} />
                                <Row label="Nomor DO" value={record.nomorDO} icon={ClipboardList} />
                                <Row label="Nomor Polisi" value={record.nomorPolisi} icon={Truck} />
                                <Row label="Plant" value={record.plant} icon={MapPin} />
                                <Row
